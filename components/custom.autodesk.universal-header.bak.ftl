...
<#else>
	...
	<div id="forums-uh-container-wrapper" style="min-height:113px;">
		<div id="forums-uh-container" style="min-height:113px;position:fixed;z-index:999;top:0;left:0;right:0;"></div>
	</div>
	...
	<#assign profile_access_token = "" />
	<#--
	<#if !user.anonymous>
		<#assign profile_access_token = rest("/users/self/settings/name/profile.access_token").value />
	</#if>
	-->
	...
	<#assign unread_messages_count = 0 />
	<#if !user.anonymous>
		<#assign unread_messages_count = rest("/users/id/${user.id?c}/mailbox/notes/inbox/unread/count").value />
	</#if>
	...
	;(function() {
		...
		var profile_access_token = '${profile_access_token?js_string}';
		var profile_access_token_exp = 0;
		if(profile_access_token) {
			try {
				var profile_access_token_exp_tmp = parseJwt(profile_access_token).exp;
				if(Number.isInteger(profile_access_token_exp_tmp)) {
					profile_access_token_exp = profile_access_token_exp_tmp * 1000;
				}
			} catch (e) {
		...
		var session_access_token = sessionStorage.getItem('uh_access_token_v2');
		var session_access_token_exp = 0;
		if(session_access_token) {
			try {
				var session_access_token_exp_tmp = parseJwt(session_access_token).exp;
				if(Number.isInteger(session_access_token_exp_tmp)) {
					session_access_token_exp = session_access_token_exp_tmp * 1000;
				}
			} catch (e) {
		...
		if((profile_access_token_exp - now_ts) > 30000) {
			uh_access_token = profile_access_token;
		}
		if((session_access_token_exp - now_ts) > 30000) {
			uh_access_token = session_access_token;
		}
		*/

		function renderUniversalHeader(accessToken, reRender) {
			...
			var meMenuConfig = {
				...
				onSignOut: function () {
					/*sessionStorage.removeItem('uh_access_token_v2');*/
		...
		function reRenderMeMenu(accessToken) {
			...
			if (accessToken) {
				event = new CustomEvent('SSO_SIGNED_IN', { "detail": { "at": accessToken } });
			} else {
				event = new CustomEvent('SSO_SIGNED_OUT');
			}
		...
		**/
		...
		function checkSession() {
			...
			var accountsDomain = '${(config.getString("phase", "") == "prod")?string("accounts.autodesk.com", "accounts-staging.autodesk.com")}';
						...
						DHXOxygen.getUserData().then(function (result) {
							if (result.CONSUMER_RESPONSE_DATA && result.CONSUMER_RESPONSE_DATA.userid) {
								if (!loggedIntoForum) {
									/*
									if(result.jwt_token) {
										sessionStorage.setItem('uh_access_token_v2', result.jwt_token);
									}
									*/
								...
								if(result.jwt_token) {
									...
									sessionStorage.setItem('uh_access_token_v2', result.jwt_token);
									if(!uh_access_token) {
										uh_access_token = result.jwt_token;
										renderUniversalHeader(uh_access_token);
										return;
									}
									*/
