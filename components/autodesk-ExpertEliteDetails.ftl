...
<#list restadmin("/users/id/${user.id?c}/roles").roles.role as role>
  <#if role.name?? && (role.name == "Administrator" || role.name == "Expert Elite Admin" || role.name == "EE Admin View-only") ||(role.name == "Expert Elite") >
    <#assign admin = true  />
  </#if>
  ...
  <#if role.name?? && role.name == "Expert Elite Admin">
      
          <#assign EEInternalAdmin = true  />
...
<#-----get preferred language of the current user---->
<#assign pref_lang = settings.name.get("profile.language") />
<#if !pref_lang?has_content>
	<#assign pref_lang = "en" />
</#if>
...
<#else>
    ...
    <@liaAddScript>
      ...
      function setSelectProductFieldOptions(selectFieldID,options)
      {
        ...
        {
            optionsDOM = optionsDOM + "<label style=\"display:block;\"><input type=\"checkbox\" name=\"product\" value='"+options[i].toUpperCase()+"'/>&nbsp;&nbsp;" + options[i].toUpperCase() + "</label>";
        }
...
//debugger;
     ...
     //debugger;   
      ...
      //var getEEMemberDetails = "${autodesk_ee_vm_baseurl}/expertelite/nominee/id/${communityUserIdFromUrl}";
        if("${communityUserIdFromUrl}"===""){
             var filterJSONForFetch={communityUserId:"${user.login}"};
                console.log(filterJSONForFetch);
                var getNomineeListURL = "${autodesk_ee_vm_baseurl}/expertelite/nominee/list";

                $.ajax({
                type: "POST",
                crossDomain: true,
                data: JSON.stringify(filterJSONForFetch),
                ...
                //debugger;
                var nomineeList=data.nomineeList;
                for(var ind=0;ind<nomineeList.length;ind++){
                      if(nomineeList[ind].memberStatus==="EE ACTIVE"){
                           forumUserId= nomineeList[ind].id; 
                            break;
                             }                                   
                        }
                    }
                });
             var getEEMemberDetails = "${autodesk_ee_vm_baseurl}/expertelite/nominee/id/"+forumUserId+"?language=${pref_lang}";                                        
        }else{
...
<#if admin>
      ...
      $("#languages").focusout(function () {
//;
        var flag=false;
        var allCheckBoxes=$("#languages input[type=checkbox]");
        for(var ind=0;ind<allCheckBoxes.length;ind++){
          if($(allCheckBoxes[ind]).prop("checked") == true){
                flag=true;                        
                }
        }  
...
//debugger;
var statusSelected = $('#Autodesk_Product_Proficiency #member_status').val().toUpperCase();
var forumUserId=$('#forum_user_id').val();
         ...
         var languagesSpoken = [];
          ...
          language_checkboxes.each(function() {     
            ...
            if(checkbox.prop("checked")){
                languagesSpoken.push(checkbox.val().toUpperCase());
            }               
          });
            
         if ($("#other_languages").val() != "") {
              languagesSpoken.push($("#other_languages").val().toUpperCase());
          }
          ...
          product_checkboxes.each(function() {      
            var checkbox = $(this);
            if(checkbox.prop("checked")){
                autodesk_proficiency.push(checkbox.val().toUpperCase());
            }               
          });
          
          if ($("#other_proficiency").val() != "") {
            autodesk_proficiency.push($("#other_proficiency").val().toUpperCase());
          }
        ...
        if($("#forum_user_id").val()!="" && $("#candidate_first_name").val() != "" && $("#candidate_last_name").val() != "" && $("#email").val() != "" && regex.test(component.input.val()) && $("#address1").val() != "" && $("#city").val() != "" && $("#country").val() != "" && languagesSpoken!="" <#if EEMember> && $("#shirt_size").val() != "" </#if> && $('input[name=is_autodesk_reseller]:checked').length > 0 && $('input[name=is_autodesk_student]:checked').length > 0)
        {
          tab1Validation= true;
        }
        else {
    ...
    //console.log("tab1Validation : "+tab1Validation);
        ...
        if (validation == "valid") {
          ...
          if ($('input[name=Send_EE_Communications]:checked').length > 0) {
            if ($('input[name=Send_EE_Communications]:checked').val() == "true") {
          ...
          var emailComm=$("#email2").val();
          if(emailComm==undefined){
              emailComm="";
              }
          
          if(include_in_directory===undefined || isSubmitterEEMember===undefined || Send_EE_Communications===undefined){
              include_in_directory=null;
              isSubmitterEEMember=null;
              if(Send_EE_Communications===undefined){
              Send_EE_Communications=false;
              }
          ...
          payload = {
            "personalDetails": {
              "id": $("#id").val(),
              "submitterFullName": submitterFullName.toUpperCase(),
              "isSubmitterEEMember": isSubmitterEEMember,
              "nominationSubmissionDate": nominationSubmissionDate,
              "firstName": $("#candidate_first_name").val().toUpperCase(),
              "lastName": $("#candidate_last_name").val().toUpperCase(),
              "email": $("#email").val().toUpperCase(),
              "email2": emailComm.toUpperCase(),
              "sendEECommunications": Send_EE_Communications,
              "phone": $("#phone_number").val(),
              "address1": $("#address1").val().toUpperCase(),
              "address2": $("#address2").val().toUpperCase(),
              "city": $("#city").val().toUpperCase(),
              "stateOrProvince": $("#state").val().toUpperCase(),
              "postalCode": $("#postal_code").val().toUpperCase(),
              "country": $("#country").val().toUpperCase(),
              "languageSpoken": languagesSpoken.join(","),
              "shirtSize": $("#shirt_size").val().toUpperCase(),
              "autodeskCommunityUserId": community_id,
              "autodeskResellerName": reseller_name.toUpperCase(),
            ...
            "proficiency": {
              ...
              "memberStatus": $("#member_status").val().toUpperCase(),
...
//debugger;
window.status_flag=true;
            $.ajax({ 
              type: "POST",
              url:"https://forums${stage}.autodesk.com/autodesk/plugins/custom/autodesk/autodesk/update_role_expert_elite?user_id="+forumUserId+"&status="+statusSelected,
              dataType: "json",
              async: false,
...
//debugger;
        ...
        var allCountryList=$('#country option');
        $(allCountryList).each(function(index,curr){
             
              if($(curr).text().toLowerCase()===payload1.personalDetails.country.toLowerCase()){
              $("#country").val($(curr).text());
              }
            });
...
;
        var shirtsize=payload1.personalDetails.shirtSize;
        if(shirtsize==null){
              shirtsize="";
              }
        $("#shirt_size").val(shirtsize.toUpperCase());
        ...
        for (var i = socialMediaPresenceValues.length - 1; i >= 0; i--) {
          $("#"+socialMediaPresenceValues[i].name.toLowerCase()).val(socialMediaPresenceValues[i].handle);
        };
      ...
      $('table input[type=text]').each(function(index,curr){
        $(curr).blur(function(e){
        //debugger;
        var tar=e.target;
        if($(tar).attr('id')!=="forum_user_id"){
        var vale=$(tar).val();
        $(curr).val(vale.toUpperCase());
        }
        });
        });
      ...
      $(".input_txt,.EESelect,input[type=radio]").change(function() {
        //console.log('changed');
        dirtyField = true;
      });
    ...
    $(window).load(function() {
      ...
      <#-- Disable all fields if user have read-only access--->
        ...
        <#if EEInternalAdmin>
            $('.expert-elite-internal_notes textarea').prop("disabled",false);
        </#if>
        <#if EEMember && !adminViewOnly && !EEInternalAdmin>
       // console.log($(EEMember)+" " +$(adminViewOnly)+" " +$(EEInternalAdmin));
        $('.expert-elite-internal_notes').hide();
         $('.expert-elite-ee-mentor').hide();
         $('.expert-elite-ee-nomination-submitted').hide();
         $('.expert-elite-ee-nominatedby').hide();
         $('.expert-elite-ee-nomination-date').hide();
         $('.expert-elite-member-status').hide();
         $('.expert-elite-date-joined').hide();
         $('.expert-elite-external-comments').hide();
        $('.ee-other-lang-mentor').hide();  
        </#if>
    ...
    <div class="container">
      ...
      <div class="InfoMessage error-message-panel">
        ...
        <div class="tab">
          <a class="tablinks" id="defaultOpen" onclick="viewData(event,'Expert_Elite_Member_Information')">Expert Elite ${text.format('autodesk.ee.member-information')}</a>
          <a class="tablinks" onclick="viewData(event,'Autodesk_Product_Proficiency')">${text.format('autodesk.ee.product-proficiency-engagement')}</a>
            <#if EEInternalAdmin || adminViewOnly>
          ...
          <div class="panel-body" id="Autodesk_Product_Proficiency">
              <table>
                ...
                <tr class="expert-elite-ee-mentor">
                  <td>
                ...
                <tr class="expert-elite-internal_notes">
                      <td>
                ...
                <tr class="expert-elite-ee-nomination-submitted">
                  <td>
                ...
                <tr class="expert-elite-ee-nominatedby">
                    <td>
                ...
                <tr class="expert-elite-ee-nomination-date">
                  <td>
                ...
                <tr class="expert-elite-member-status">
                  <td>
                ...
                <tr class="expert-elite-date-joined">
                  <td>
                ...
                <tr class="expert-elite-external-comments">
                  <td>
          ...
          <#if admin>
              ...
              <div class="panel-body" id="Point_Values" style="display:none">
               <table class="">
                <thead>
                  <tr>
                    <th>Attribute</th>
    ...
    <div style="margin-top:15px;">
      ...
      <a class="btn-brand " id="cancelbtn" href="/t5/custom/page/page-id/ExpertEliteSearch">${text.format('autodesk-EE-detailsPage-button-cancel')}</a>
    </div>
