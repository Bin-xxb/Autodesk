<#import "autodesk-forumpagetab-panel-macro-unresolved-v2.ftl" as autodesk>
<#-- OOTB components --
...
<div class="lia-tabs-standard-wrapper custom-component-threads-tab-list lia-component-tabs" id="tabgroup">
   <ul class="lia-tabs-standard">
   	...
   	<#list panelTabs as tab>
		...
		<#if tab.id == "all-posts" >
			<li class="lia-tabs lia-tabs-active" data-tab="${tab.id}-tab-content" id="all-posts-tab">
				...
				<span><a class="lia-link-navigation tab-link lia-custom-event" tabindex="${tab_index}" href="${AllpostsURL}">${tab.title}</a></span>			
			</li>
		<#else>
    ...
    <#else>
             ...
             <div class="custom-component-message-list-wrapper tab-content lia-js-hidden" id="${panelTabs[2].id}-tab-content">
                ...
                <#if (messages?size > 0)>
                        ...
                        <div class="t-data-grid thread-list">
                            ...
                            <@autodesk.lib_custom_wide_table_posts_with_avatar messages=messages optionsHash={"pageSize"    :messages_per_page_linear} />
        ...
        <#elseif !currentURL?contains('solved-posts-page') && !currentURL?contains('unanswered-posts-page') && currentURL?contains('faq-posts-page')>
            ${componentCacheSupport.setCacheVariation("board_id", coreNode.id)}
            <@component id="autodesk-forumTabs-faq"/>       
        </#if>
    </#if>  
           
</div>

<#if currentURL?contains('solved-posts-page') || currentURL?contains('unanswered-posts-page')>
...
$(document).ready(function($) {
    ...
    //debugger;
                               ...
                               if(tab_id=="all-posts"){
                                    panelTabs.renderMessageHoverAllPosts(tab_id+'-tab-content');
                                }else{
                                   var toolTipElem=$('#lia-panel-tooltip');
    ...
    renderMessageHoverAllPosts:function(tab_des){
                $('body').append('<div id="lia-panel-tooltip"></div>');
                $("#lia-panel-tooltip").append("<div id='tt' ></div>");
