 <@liaMarkupCache ttl="300000" variation="board_id" anonymousOnly="false" />
...
<#assign faqPostPage = http.request.parameters.name.get('faq-posts-page', '1')?string?number />
...
<#assign optionsHash={'location'  :'boards',
                    'locationId':boardId,
...
<#-- function to merge two objects and hash2 values are overrided for same keys-->    
...
<#-- count of solved posts required for frequently asked , it is same for solved and faq -->  
<#function getLastestSolutionsCount optionsHash>
<#assign   defaultOptions = {
    ...
    <#assign count = rest(queryString).value?number />
    <#return count>
</#function>    
        
<#function getFrequentlyAskedQuestions optionsHash>
    <#assign messages = getSolvedPosts(optionsHash) />
    <#return messages>
</#function>

    
<#-- function to get all solved posts sorted by views , function used by freq asked questions-->       
<#function getSolvedPosts optionsHash>
    <#assign
        defaultOptions = {
            "location"  :"",
            "locationId":"",
    ...
    <#return messages>
        ...
        <div class="custom-component-message-list-wrapper tab-content lia-js-hidden" id="faq-posts-tab-content">
            <#assign pagingLabel = "faq-posts-page" />
            <#assign pageNumber = faqPostPage />
            <#assign messages=getSolvedPosts(optionsHash)/>
            <#assign count=getLastestSolutionsCount(optionsHash)/>
            <#-- NOTE : Hide pagination and show only 1st page data for faq-posts tab --> 

            <div class="MessageList lia-component-forums-widget-message-list lia-forum-message-list custom-message-list" id="messageList_faq-posts">
                    ...
                    <div class="t-data-grid thread-list">
                                ...
                                <@lib_custom_wide_table_posts_with_avatar messages=messages optionsHash={"pageSize"    :messages_per_page_linear} />
...
<#-- function to get user profile avatar on forumPage tab panel , intertnally function called in avatarlib macro -->    
<#function getUserProfileAvatar user_id>
<#assign profileAvatar = "" />
<#if profileAvatar == "">
    <#attempt>
        <#if !(profileAvatar?has_content) >
...
<#-- Macro to populate DOM on ForumPageTab Panel -->    
<#macro lib_custom_wide_table_posts_with_avatar messages optionsHash>
	...
	<#assign RestCall="/boards/id/"+coreNode.id+"/subscriptions/global/float/thread"/>
	<#assign floatedMsgResponse = rest(RestCall) />
         ...
         <tbody>
            <#list messagesTemp as message>
                ...
                <#if isThread=="true">     
                        ...
                        <#assign threadResponse=rest('threads/id/${message.id}')/> 
                        <#assign messageSolved="false"/>

                        <#if threadResponse.thread.solutions.solution?size gt 0>
                             <#assign messageSolved="true"/>
                        </#if>
                            ...
                            <#assign msgBody=rest('/messages/id/${message.id}/body').value/>
                                   <td class="cMessageAuthorAvatarColumn lia-data-cell-secondary lia-data-cell-icon">
                                    ...
                                    <#assign profileAvatar = getUserProfileAvatar(authorId) />
                                      ...
                                      <#if message.author.login=="Anonymous">
                                            <img class="lia-user-avatar-message" title="${message.author.login}" alt="${message.author.login}" id="display_5" src="${profileAvatar}">
                                        <#else>
                                            <a class="UserAvatar lia-link-navigation" target="_self" href="/t5/user/viewprofilepage/user-id${authorId}"><img class="lia-user-avatar-message" title="${message.author.login}" alt="${message.author.login}" id="display_5" src="${profileAvatar}">
                                            </a>
                                        </#if>
                                      </div>
                                   </td>
                                      ...
                                      <div class="lia-component-messages-column-message-info">
                                         ...
                                         <div class="lia-info-area">
                                          ...
                                          <#-- apply app cache to Forum Page ends -->					
                                            ...
                                            <#assign replyAuthorId=response_latest_post.data.items[0].author.id/>
                                           ...
                                           <#if response_latest_post.data.items[0].post_time_friendly??>
                                            ...
                                            </span> ${text.format("general.by")} 
                                                <#assign appcache_rankingReply = "autodesk" + ("_users_id_" + replyAuthorId) + "_ranking" />
                                                  <#assign rankingReply = appcache.get(appcache_rankingReply, "") />
                                                  <#if !(rankingReply?has_content) >
                                                  <#assign rankingReply = restadmin("/users/id/" + replyAuthorId + "/ranking").ranking />
                                                  <#assign ranking_cacheReply = appcache.put(appcache_rankingReply, rankingReply) />
                                                  <@debug msg="fresh call for ranking for ${appcache_rankingReply}" />
                                                  <#else>
                                                  <@debug msg="ranking taken from appcache for ${appcache_rankingReply}"/>
                                                  </#if>
                                                <span class="UserName lia-user-name lia-user-rank-Participant lia-component-common-widget-user-name">
                                            <#if rankingReply.display.left_image?has_content && rankingReply.display.left_image.url?has_content >
                                            <img class="lia-user-rank-icon lia-user-rank-icon-left" title="${rankingReply.name?html}" alt="${rankingReply.name?html}" src="${rankingReply.display.left_image.url}">
                                            </#if>
