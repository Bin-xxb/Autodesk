<#--Developed by Peter Lu peter.lu@hinterlands.com.au 2020-10-02-->
<div id="xp-me-menu-wrapper">
	<div id="xp-me-menu"></div>
</div>
...
<#if config.getString("phase", "") == "prod">
	<script type="text/javascript" src="https://memenu.xp.autodesk.com/meMenu.js"></script>

	<#assign DHXOxygen_environment = "production" />
	<#assign MeMenu_environment = "prd" />
	<#-- Commented out due to the SSO upgrade 2021
	<#assign bounceUrl = "/auth/openid_security_check?openid_identifier=https%3A%2F%2Faccounts.autodesk.com%2F&action=bounce&dest_url=${http.request.url?url}" />
	-->
<#else>
	<script type="text/javascript" src="https://memenu.xp-stg.autodesk.com/meMenu.js"></script>

	<#assign DHXOxygen_environment = "staging" />
	<#assign MeMenu_environment = "stg" />
	<#-- Commented out due to the SSO upgrade 2021
	<#assign bounceUrl = "/auth/openid_security_check?openid_identifier=https%3A%2F%2Faccounts-staging.autodesk.com%2F&action=bounce&dest_url=${http.request.url?url}" />
	-->
</#if>


<#assign oxygenOpenidResponseUrl = "https://${http.request.serverName}/html/assets/oxygen_openid_response_2020.html" />

<#assign loggedIntoForum = user.anonymous?string("false", "true") />

<#assign logoutUrl = webUi.getUserLogoutPageUrl("/") />
<#assign loginUrl = webUi.getUserLoginPageUrl(http.request.url) />
...
;(function() {
	...
	var DHXOxygen = window.DHXOxygen || {};
	// Commented out due to the SSO upgrade 2021
	//var bounceUrl = '${bounceUrl?js_string}';

	var loggedIntoForum = ${loggedIntoForum};

	var logoutUrl = '${logoutUrl?js_string}';

	var loginUrl = '${loginUrl?js_string}';

	var forumSsoId = '${sso_id?js_string}';

	var lang = '${lang?js_string}';

	var MeMenu_environment = '${MeMenu_environment?js_string}';

	var DHXOxygen_environment = '${DHXOxygen_environment?js_string}';

	var oxygenOpenidResponseUrl = '${oxygenOpenidResponseUrl?js_string}';

	var meMenuInit = function(jwt) {
		...
		var meMenuContainer = document.getElementById("xp-me-menu");

		// more details about eventHandler below
		var eventHandler = (params) => {
			if (params.type === "error") {
				// Any additional error handling - site specific
				console.error("Me Menu", params.event, params.message);
			} else {
		...
		if (window.adsk && window.adsk.meMenu) {
			var options = {
				siteId: "forums",
				iconSize: "small",
				language: lang,
				environment: MeMenu_environment,
				onSignIn: function () {
				...
				useMock: false, // Right now useMock defaults to true for testing till accessToken is available. We will be removing this param  later.
			...
			window.adsk.meMenu.render(meMenuContainer, options);
		}
	...
	if (typeof DHXOxygen.setOptions === 'function') {
		DHXOxygen.setOptions({
			oauth2: true,
			environment: DHXOxygen_environment, // 'production', 'staging', 'dev'
					...
					DHXOxygen.getUserData().then(function (result) {
						if (result.CONSUMER_RESPONSE_DATA && result.CONSUMER_RESPONSE_DATA.userid) {
							if (!loggedIntoForum) {
								...
								if(result.CONSUMER_RESPONSE_DATA.userid !== forumSsoId) {
									window.location.href = logoutUrl;
									return;
								} else {
									if (result.jwt_token) {
										meMenuInit(result.jwt_token);
										return;
									} else {
										meMenuInit();
										return;
