	...
	;(function() {
		...
		function renderUniversalHeader(accessToken) {
			...
			var meMenuConfig = {
				...
				onSignOut: function () {
					sessionStorage.removeItem('uh_access_token');
			...
			var config = {
				...
				eventListener: function (response) {
					if (response) {
						if (response.event === 'USER_INFO_FETCH_FAILED' || response.event === 'LINKS_FETCH_FAILED') {
							sessionStorage.removeItem('uh_access_token');
							meMenuConfig.onSignOut();
						}
					}
				}
			};
		...
		function initDHXOxygen() {
			...
			//need oAuth2 bounce url, instead of oAuth1
			//var bounceUrl = '/auth/openid_security_check?openid_identifier=https%3A%2F%2F' + accountsDomain + '%2F&action=bounce&dest_url=${http.request.url?url}';
					...
					if (params.loginNeeded == "true") {
						...
						DHXOxygen.getUserData().then(function (result) {
							if (result.CONSUMER_RESPONSE_DATA && result.CONSUMER_RESPONSE_DATA.userid) {
								if (!loggedIntoForum) {
									sessionStorage.setItem('uh_access_token', result.jwt_token);
									//Commented out due to the SSO upgrade 2021
									...
									if(result.CONSUMER_RESPONSE_DATA.userid !== forumSsoId) {
										...
										if (result.jwt_token) {
											sessionStorage.setItem('uh_access_token', result.jwt_token);
											renderUniversalHeader(result.jwt_token);
											return;
										} else {
											sessionStorage.removeItem('uh_access_token');
											renderUniversalHeader(null);
		...
		var cachedAccessToken = sessionStorage.getItem('uh_access_token');
		if(cachedAccessToken === null) {
			if(profile_access_token) {
				cachedAccessToken = profile_access_token;
				sessionStorage.setItem('uh_access_token', profile_access_token);
				renderUniversalHeader(cachedAccessToken);
			} else {
				initDHXOxygen();
			}
		} else {
